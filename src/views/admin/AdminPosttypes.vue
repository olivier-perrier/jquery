...
export default {
  ...
  methods: {
    createPost: function() {
      axios
        .post(
          "http://localhost:3000/API/posttypes/create?postTypeId=" + this.customType._id,
          {}
        )
        .then(response => {
          this.posts.push(response.data.post);
          this.$router.push(
            "/admin/posttypes/" + response.data.post._id
          );
        });
    },
    deletePost: function(postId) {
      axios
        .delete(
          "http://localhost:3000/API/posttypes/" + postId
        )
        .then(() => {
    ...
    mountType: function() {
      // Id du type custom dans la table custom
    ...
    mountPosts: function() {
      // Récupération de tous les posttypes
      axios
        .get("http://localhost:3000/API/posttypes")
        .then(response => {
